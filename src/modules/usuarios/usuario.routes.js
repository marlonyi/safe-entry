...
const { loginRateLimiter } = middlewares.rateLimit;
const { verificarToken, esAdmin, esPorteriaOAdmin, esSuperAdmin } = middlewares.auth;
const { verificarLimiteUsuarios } = middlewares.planLimits;
const { validateLogin, validateCreateUser, validateMongoId } = middlewares.validation;
const usuarioController = require('./usuario.controller');

// ========================================
// 📌 Rutas públicas (sin autenticación)
// ========================================
router.post("/login", loginRateLimiter, validateLogin, usuarioController.loginUsuario);
...
router.post("/crear", verificarToken, esPorteriaOAdmin, verificarLimiteUsuarios, validateCreateUser, usuarioController.crearUsuario);
...
// ========================================
router.get("/:id", verificarToken, validateMongoId, usuarioController.obtenerUsuarioPorId);
router.put("/:id", verificarToken, validateMongoId, usuarioController.actualizarUsuario);
router.delete("/:id", verificarToken, esPorteriaOAdmin, validateMongoId, usuarioController.eliminarUsuario);
